...
//Se descomenta para la nube heroku
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Producto añadido satisfactoriamente..!'];
						
					}else{
						return ['status'=> 303, 'message'=> 'Fallo al ejecutar query'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Fallo al cargar imagen'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Imagen muy grande, Tamaño Maximo 2MB'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Formato de imagen invalido [Formatos Validos : jpg, jpeg, png]'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce-app-h/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Producto modificado satisfactoriamente..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Fallo al ejecutar query'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Fallo al subir imagen'];
				}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Producto actualizado satisfactoriamente'];
			}else{
				return ['status'=> 303, 'message'=> 'Fallo al ejecutar query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=> 'Id de Producto Invalido'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nueva categoria añadida satisfactoriamente'];
			}else{
				return ['status'=> 303, 'message'=> 'Fallo al ejecutar query'];
			}
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Producto removido del stock'];
			}else{
				return ['status'=> 202, 'message'=> 'Fallo al ejecutar query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Id de producto invalida'];
		}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'Id de categoria invalido'];
		}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Categoria actualizada'];
			}else{
				return ['status'=> 202, 'message'=> 'Fallo al ejecutar query'];
			}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nueva marca añadida satisfactoriamente'];
			}else{
				return ['status'=> 303, 'message'=> 'Fallo al ejecutar query'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			return ['status'=> 303, 'message'=>'Id de marca invalido'];
		}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Error de sesion']);
	}
}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Sesion invalida']);
	}
